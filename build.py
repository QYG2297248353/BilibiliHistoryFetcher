    ...
    try:
        ...
        if os.name == 'nt':  # Windows
            ...
            if not os.path.exists(yutto_exe):
                # 尝试在系统路径中查找
                try:
                    yutto_exe = subprocess.check_output(['where', 'yutto'], text=True).strip().split('\n')[0]
                    print(f"\n在系统路径中找到 yutto: {yutto_exe}")
                except (subprocess.CalledProcessError, FileNotFoundError):
                    print(f"\n警告: 找不到 yutto.exe: {yutto_exe}")
                    print("将尝试继续构建，但可能会缺少yutto功能")
                    yutto_exe = None
            else:
            ...
            if not os.path.exists(yutto_exe):
                # 尝试在系统路径中查找
                try:
                    yutto_exe = subprocess.check_output(['which', 'yutto'], text=True).strip()
                    print(f"\n在系统路径中找到 yutto: {yutto_exe}")
                except (subprocess.CalledProcessError, FileNotFoundError):
                    print(f"\n警告: 找不到 yutto: {yutto_exe}")
                    print("将尝试继续构建，但可能会缺少yutto功能")
                    yutto_exe = None
            else:
                print(f"\n找到 yutto: {yutto_exe}")
        ...
        try:
            ...
            if yutto_exe and os.path.exists(yutto_exe) and os.path.exists(dist_dir):
                try:
                    shutil.copy2(yutto_exe, dist_dir)
                    print(f"\n已复制 yutto 到 {dist_dir}")
                except Exception as e:
                    print(f"\n复制 yutto 时出错: {e}")
            elif not os.path.exists(dist_dir):
                print(f"\n警告: 找不到输出目录 {dist_dir}，打包可能失败")
            elif not yutto_exe or not os.path.exists(yutto_exe):
                print(f"\n警告: 找不到 yutto，将跳过复制步骤")
...
def create_spec(source_spec, target_spec, venv_site_packages=None):
    ...
    try:
        ...
        if "(os.path.join(venv_site_packages, 'yutto'), 'yutto')," in content:
            content = content.replace(
